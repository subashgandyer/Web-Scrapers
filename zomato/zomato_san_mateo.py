...
conn = sqlite3.connect('zomato_san_mateo.db')
...
def zomato_spider(max_pages):
	...
	while page <= max_pages:
		if sleep_counter != 5:
			...
			url = "https://www.zomato.com/san-mateo-ca/san-mateo-restaurants?page="+str(page)
			print 'Printing URL .....   ', url
			header = {'User-Agent': 'Mozilla/5.0'} 
			soup = get_soup(url,header)
			#print 'Printing soup .....   \n',soup
			...
			for link in soup.findAll('div',{'class':'search-snippet-card'}):
				...
				if location_txt:
					rest_location = location_txt.string.strip()
				...
				if address_txt:
					rest_address = address_txt.string.strip()
				...
				if rating_txt:
					rest_rating = rating_txt.string.strip()
				...
				if votes_txt:
					rest_votes = votes_txt.string.strip()
					if rest_votes:
				...
				if hours_txt:
					rest_hours = hours_txt.string.strip()
